...
async function recommendation() {
      ...
      genres.forEach((moviegenre: moviegenre) => {
        if (movie.genres.some((genre) => genre.genre === moviegenre.genre)) {
          if (moviegenre.genre in userPreference)
            newScore.score +=
              (userPreference[
                moviegenre.genre as keyof typeof userPreference
              ] as number) / movie.genres.length;
        }
      });
      if (movie.imdb_rating >= userPreference.imdb_rating) {
        newScore.score += 5;
      }
      if (new Date(movie.release_date) >= new Date(userPreference.release_year))
        newScore.score += 5;

      const existingScore = scoreArray.find(
        (score) =>
          score.user_id === newScore.user_id &&
          score.movie_id === newScore.movie_id,
      );
      if (existingScore) {
  ...
  await Promise.all(
    scoreArray.map(async (rec) => {
      const recommendations = await db.recommendation.create({ data: rec });
    }),
  );
}
